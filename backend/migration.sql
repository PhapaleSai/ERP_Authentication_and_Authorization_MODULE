...
-- Adds audit fields to all tables and creates the user_tokens table
...
-- 5. Add audit fields to students (legacy) table
...
-- 6. Ensure 'guest' and 'admin' roles exist
INSERT INTO roles (role_name, description, created_by, created_from)
VALUES 
    ('guest', 'Default role with minimum permissions', 'system', 'migration')
ON CONFLICT (role_name) DO NOTHING;

INSERT INTO roles (role_name, description, created_by, created_from)
VALUES 
    ('admin', 'Administrator with full access', 'system', 'migration')
ON CONFLICT (role_name) DO NOTHING;
